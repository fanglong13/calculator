...
function buttonClick(event) {
  ...
  switch (action) {
    ...
    case 'backspace':
      backspace();
      break;
    // Add the result to the expression as a startig point if expression is empty
    case 'addition':
    case 'subtraction':
    case 'multiplication':
    case 'division':
...
function addValue(value) {
  if (value === '.') {
    ...
    // Checks if this is the first decimal in the current number or if the expression is empty
    if (
...
function negate() {
  ...
  if (expression === '' && result !== '') {
    result = -result;
    // Toggle the sign of the expression if its not alredy negative and its not empty
...
function percentage() {
  // Evaluate the expression, else if it will take the percentage of only the first number
  if (expression !== '') {
    result = evaluateExpression();
    expression = '';
